...
def test_search_normal_mult_assoc(monkeypatch, setup_mdb_dir):
    ...
    args_expected = [
            ...
            (
                (
                    ...
                    {"downloaded": "0"},
                    # int assoc
                    {"tag": ["Nakadashi", "Ahegao", "Femdom"]},
                    # ex assoc
                    {},
                    # order_by,     limit, after, before
                    "Books.id DESC", 5, None, (15,)
                ),
                [17]
            ),
            (
                (
                    ...
                    {"downloaded": "1"},
                    # int assoc
                    {"tag": ["Nakadashi", "Ahegao"]},
                    # ex assoc
                    {},
                    # order_by,     limit, after, before
                    "Books.id ASC", 5, (2,), None
                ),
                [3, 6]
            )
        ]
    ...
    for args, expected in args_expected:
        ...
        # iterate over expected since no rows returned would just pass otherwise
        for i, exp_id in enumerate(expected):
            assert rows[i]["id"] == exp_id
