...
# -*- coding: utf-8 -*-
"""
File: tsumino_col_info.py
Description: Contains functions to convert a windows dir listing textfile into a dictionary.
             Book titles can then be looked up in the dictionary using their titles with
             get_info_by_title function.
"""
...
def import_obj_from_json(filename):
    ...
    # 2) converting dictionary to a list of [(k1,v1),(k2,v2)] format while encoding it using json,
    #    and converting it back to dictionary after decoding it back
    # 3) add option for dict to decoder func:
    # if isinstance(x, dict):
    #         return {int(k):v for k,v in x.items()}
    # 4) use pickle -> better/faster for only python anyways but unsecure, which doesnt matter
    #    here since only b64_md5s and int/float sizes will be stored or use YAML
...
FILE_LINE_RE = re.compile(
    r"(\d{2}\.\d{2}\.\d{4})\s+(\d{2}:\d{2})\s+([0-9\.]+)\s+([^\n\r:<>|?*\\\/\"]+)\.(\w+)"
)
...
def create_info_from_dirlist(fn):
    ...
    for ln in inp.splitlines():
        if "<DIR>" in ln:
            ...
            cur_dir_dict["dirsize"] = int(dirsize_str.strip().split(
                " ", 1)[0].replace(".", ""))
            ...
            title_eng = re.sub(BRACKETS_RE, "", title_eng
                               if title_eng else dirname).strip()
            ...
            if ext in ("jpg", "png", "gif"):
                ...
                cur_dir_dict["max_pg_nr"] = max(pg_nr,
                                                cur_dir_dict.get(
                                                    "max_pg_nr", 0))
                imgs += 1
            elif fname == "tags":
